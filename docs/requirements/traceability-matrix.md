...
| L1-APP-040    | Platform abstraction          | L2-PLAT-010 thru -060                           | `platform/adapter.ts`, `src-tauri/lib.rs` | I, A   | Implemented   |
| L1-APP-050    | Dual deployment               | L2-DEPLOY-010 thru -040                         | `vite.config.ts`, `tauri.conf.json`, `Cargo.toml` | D   | Implemented   |
| L1-FILE-010   | Open Ch10 files               | L2-FILE-010, -020, -030, -040                   | `core/file.rs`, `core/index.rs`, `lib.rs` | T, D   | Implemented   |
| L1-FILE-020   | Parse TMATS                   | L2-FILE-020                                     | `core/file.rs`, `core/summary.rs`         | T      | Implemented   |
| L1-FILE-030   | Build channel index           | L2-LOAD-010 thru -050                           | `core/index.rs`, `main.ts`                | T, D   | Implemented   |
| L1-FILE-040   | Validate checksums            | L2-PROP-030                                     | `core/checksum.rs`, `core/index.rs`       | T      | Implemented   |
...
| L1-VIS-010    | Waveform display              | L2-VIS-020, -030, -040                          | `viewport.ts`                             | D      | Demo data     |
...
1. **L1-STD-010/020/030** — Blocked on completion of `irig106-time` and `irig106-tmats` crates and spec-level decoder knowledge.
2. **L1-VIS-010 (real waveform)** — Currently renders demo data; needs waveform engine backed by decoded channel data (Phase 5).
3. **L1-DIAG-030** — Time correlation diagnostics are static mock data; needs `irig106-time` correlation engine.
